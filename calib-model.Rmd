...
and [**raster**](https://CRAN.R-project.org/package=raster) packages into your current R environment, using
...
Reconstitute the file and folder structure for a calibrated model run.
...
There are four files used in the water-balance calculation that reside in the "archive" folder.
...
And the "model.rda" file contains R objects `d.in.mv.ave`, `misc`, `reduction`, `rs`,
...
At some future date all four water-balance model input files will be added to the model archive.

You can run the water-balance model and write quality assurance tables to disk ("archive/qa-\*.csv");
however, this step is unnecessary because the water-balance input files have not changed.

```{r water_budget_1, results="hide"}
UpdateWaterBudget("archive", "wrv_mfusg", qa.tables = "english")
```

The result of running the water-balance model is a new MODFLOW Well Package file ("archive/wrv_mfusg.wel").
...
You can double-click the batch file to run the model; however,
this step is unnecessary because the model input files have not changed.

## Post-Process Model

These instructions describe an example analysis of model output.

Load R objects stored in the R-data file ("archive/model.rda") into your current R environment.
...
Aggregate each wells spatial location, measured head, simulated head, and residual head data.
...
Draw the point location of the well site within the aquifer system.
