...
Each time we create a modified dataset it will mask the previously loaded version.

Next, commands from the Appendix C R-source-code file ("sir20165080_AppendixC.R"),
parse and evaluate the preamble.
...
Specify the location where the modified unprocessed-data files currently residing under ("inst/extdata").
...
Code chunks shown below are copy/pasted from Appendix C R-source file
...
external representations as R object in "data".
...
The R script for processing the model (shown in code blocks) is
...
Read Appendix D R-source-code file ("sir20165080_AppendixD.R") and remove all lines with the `invisible(dev.off())` command.
...
This R-data file contains the following objects: `rs`, `misc`, `trib`, `tr.stress.periods`, and `ss.stress.periods`.
A description of each R object is given in the help documentation of the `UpdateWaterBudget` function in the **wrv** package.
...
```{r fig_1, fig.width=fin.graph[1], fig.height=fin.graph[2], eval=TRUE}
dir.run <- file.path(getwd(), "model")
line <- grep("## ----table_budget", app.d.r)
eval(parse(text = app.d.r[1:line]))
```
...
```{r fig_2, echo=FALSE, fig.width=fin.map.s[1], fig.height=fin.map.s[2], eval=TRUE}
