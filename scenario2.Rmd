...
```{r, include=FALSE}
knitr::opts_chunk$set(eval=FALSE, dpi=100, fig.path="figures/")
```
...
```{r pre_5, results="hide", eval=FALSE}
...
Overwrite model input files with their calibrated values in the "archive" folder.
These inlcude the reference data files ("model/\*.ref") containing
 conductivity and storage coefficient values,
MODFLOW River file ("model/\*.riv"), MODFLOW Drain file ("model/\*.drn"), and
MODFLOW Layor Property Flow file ("model/\*.lpf").
...
```{r calibrated_1, results="hide", eval=TRUE}
dir.archive <- file.path(getwd(), "archive")
files <- list.files(dir.archive, full.names = TRUE)
files <- files[grep("(\\.ref|\\.riv|\\.drn|\\.lpf|eff\\.csv|seep\\.csv|trib\\.csv)$", files)]
...
```{r rda_1, results="hide", eval=TRUE}
...
d <- data.frame(flow.direction, flow.components, flow.arch, flow,
...
columns <- c("", "Component", "Archived rate<br>(acre-ft/yr)",
             "Modified rate<br>(acre-ft/yr)",
...
FUN <- function(usr, credit=NULL, max.dev.dim=c(21, 56), add.legend=FALSE) {
  Pal <- function(...) rev(viridis::viridis(...))
  compare.date <- "2007-08-16"
  r <-  rs.wt[[compare.date]] - rs.wt.arch[[compare.date]]
  r <- crop(r, extent(usr))
